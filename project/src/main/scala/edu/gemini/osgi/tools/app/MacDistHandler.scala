...
case class MacDistHandler(jre: Option[String], jreName: String)
    extends DistHandler {
  ...
  def build(
      ...
      d: Configuration.Distribution,
      solution: Map[BundleSpec, (File, Manifest)],
      log: sbt.Logger,
      appProjectBaseDir: File
  ): Unit = {
    ...
    val entitlements =
      new File(
        new File(new File(appProjectBaseDir, "dist"), "MacOS"),
        "entitlements.plist"
      )
    ...
    val plist = InfoPlist(
      ...
      version,
      fwJar,
      fwMf.getMainAttributes.getValue("Main-Class"),
      config.props,
      config.vmargsWithApp(name),
      config.icon,
      jreName
    )
    write(new File(contentsDir, "PkgInfo")) {
      _.write(Array[Byte]('A', 'P', 'P', 'L', '?', '?', '?', '?'))
    }
    write(new File(contentsDir, "Info.plist"))(os =>
      closing(new PrintWriter(os))(plist.write)
    )

    // JavaApplicationStub into macos
    val jas = new File(macosDir, MacDistHandler.launcher)
    write(jas) { os =>
      closing(getClass.getResourceAsStream(MacDistHandler.launcher))(
        stream(_, os)
      )
    }
    ...
    jre.foreach { path =>
      jreDir match {
        ...
        case Some(jreDir) =>
          ...
          jrePath.listFiles.foreach { f: File =>
            recursivecopy(f, mkdir(jrePluginDir, jreName))
          }
          ...
          val jdkInfo =
            new File(new File(jrePluginDir, jreName), MacDistHandler.jdkInfo)
          Files.copy(
            getClass.getResourceAsStream(MacDistHandler.jdkInfo),
            jdkInfo.toPath
          )
      }
    }
    ...
    if (canSign) {
      ...
      // Now sign the jre
      println(s"Signing jdk files $jrePluginDir/$jreName")
      if (
        Seq(
          "codesign",
          "--deep",
          ...
          "-s",
          MacDistHandler.certificateHash,
          new File(jrePluginDir, jreName).getAbsolutePath
        ).! != 0
      ) {
      ...
      println(s"Signing dmg file ${appDir.getPath}")
      if (
        Seq(
          "codesign",
          "-f",
    ...
    // Now create the DMG
    val volname =
      "%s_%s".format(meta.executableName(version), d.toString.toLowerCase)
