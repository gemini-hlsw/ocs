...
/**
 * This table shows how much time was charged to each program account on each
 * night. It rolls up all visits on a given night, dividing the charged time
 * equally among all partners that contributed time. DD and SV programs are
 * simply charged to DD and SV accounts rather that partner countries.
 * Non-charged time is charged to RPT.
 * @author rnorris
 */
@SuppressWarnings("unchecked")
public class TimeAccountingDetailsTable extends AbstractTable {

	@SuppressWarnings("unused")
	private static final Logger LOGGER = Logger.getLogger(TimeAccountingDetailsTable.class.getName());
	private static final long serialVersionUID = 1L;
	private static final float MS_PER_HOUR = 1000 * 60 * 60;
	private static final String DISPLAY_NAME = "Time Accounting Details";
	private static final String SHORT_DESCRIPTION = "Nightly time charges by program/account.";
	...
	public TimeAccountingDetailsTable() {
...
//		try {
			...
			String sid = id.stringValue().toUpperCase();
			...
			final Site site = ReportUtils.getSiteDesc(id);
			if (site == null) {
			...
			Map<ObservingNight, Set<ObsVisit>> nightVisits = new HashMap<ObservingNight, Set<ObsVisit>>();
			Map<ObservingNight, Set<SPComponentType>> nightInstruments = new HashMap<ObservingNight, Set<SPComponentType>>();
			Map<ObsVisit, ChargeClass> visitChargeClass = new HashMap<ObsVisit, ChargeClass>();
			for (ISPObservation obsShell: programShell.getAllObservations()) {
				...
				final ObsLog obsLog = ObsLog.getIfExists(obsShell);
				if (obsLog == null) continue;
				...
				for (ObsVisit visit: obsLog.getVisits()) {
					...
					// Remember the charge class for this visit.
					visitChargeClass.put(visit, defaultChargeClass);
					...
					visits.add(visit);
					if (instrument != null)
						instruments.add(instrument.getType());
			...
			for (Entry<ObservingNight, Set<ObsVisit>> entry: nightVisits.entrySet()) {
				...
				double prg = 0.0, cal = 0.0, rpt = 0.0;
				for (ObsVisit visit: entry.getValue()) {

					// Get the charges for this visit.
					final ChargeClass defaultChargeClass = visitChargeClass.get(visit);
				...
				appendRows(rows, sid, ratios, night, instrumentsString, prg, cal, rpt, null);
			...
			for (ISPObservation obsShell: programShell.getAllObservations()) {
				...
				for (ObsTimeCorrection corr: obs.getObsTimeCorrections()) {
					...
					appendRows(rows, sid, ratios, night, instrumentString, prg, cal, rpt, corr.getReason());
...
//		}
	...
	private void appendRows(List<Map<IColumn, Object>> rows, String sid, final Map<TimeAcctCategory, Double> ratios, final ObservingNight night, final String instrumentsString, double prg, double cal, double rpt, String comment) {

        // Write out a row for each partner, if any time was
        ...
        // We also need to write a row for non-charged time, if there
		// was any.
		if (rpt != 0.0) {
			Map<IColumn, Object> row = createRow();
			row.put(Columns.DATE, night.getNightString());
			row.put(Columns.PROGRAM_ID, sid);
			row.put(Columns.INSTRUMENT, instrumentsString);
			row.put(Columns.PRG, 0.0);
			row.put(Columns.CAL, rpt);
			row.put(Columns.ACCOUNT, "RPT");
			if (comment != null) row.put(Columns.COMMENT, comment);
			rows.add(row);
