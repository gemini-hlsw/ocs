...
class GemsCatalogResultsSpec extends MascotProgress with Specification with NoTimeConversions {
  ...
  "GemsCatalogResultsSpec" should {
    "support legacy Gsaoi Search" in {
      ...
      cwfs1x.getRaDeg should beCloseTo(cwfs1.getRaDeg, 0.001)
      cwfs1x.getDecDeg should beCloseTo(cwfs1.getDecDeg, 0.001)
      cwfs2x.getRaDeg should beCloseTo(cwfs2.getRaDeg, 0.001)
      cwfs2x.getDecDeg should beCloseTo(cwfs2.getDecDeg, 0.001)
      cwfs3x.getRaDeg should beCloseTo(cwfs3.getRaDeg, 0.001)
      cwfs3x.getDecDeg should beCloseTo(cwfs3.getDecDeg, 0.001)
  ...
  def legacySearch(inst: SPInstObsComp, raStr: String, decStr: String, tipTiltMode: GemsTipTiltMode): List[GemsGuideStars] = {
    ...
    val results = new GemsCatalog().search(obsContext, GemsUtils4Java.toCoordinates(base), options, scala.None, null)
    ...
    results.asScala.zipWithIndex.foreach { case (r, i) =>
      System.out.println("Result #" + i)
      System.out.println(" Criteria:" + r.criterion)
      System.out.println(" Results size:" + r.results.size)
    }
    val gemsResults = new GemsCatalogResults().analyze(obsContext, posAngles, results, null)
  ...
  def search(inst: SPInstObsComp, raStr: String, decStr: String, tipTiltMode: GemsTipTiltMode): List[GemsGuideStars] = {
    ...
    val options = new GemsGuideStarSearchOptions(opticalCatalog, nirCatalog, instrument, tipTiltMode, posAngles)
    val results = Await.result(GemsVoTableCatalog.search(obsContext, base.toNewModel, options, scala.None, null), 5.seconds)
