...
class GemsVoTableCatalogSpec extends Specification with NoTimeConversions {
  ...
  "GemsVoTableCatalog" should {
    ...
    "calculate the optimal magnitude limit" in {
      ...
      results should be size 2
      results.head should beEqualTo((MagnitudeBand.R, MagnitudeRange(FaintnessConstraint(16), Some(SaturationConstraint(8.5)))))
      results(1) should beEqualTo((MagnitudeBand.H, MagnitudeRange(FaintnessConstraint(14.5), Some(SaturationConstraint(7.3)))))
    }
