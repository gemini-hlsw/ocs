...
class CatalogSearchCriterionSpec extends Specification with Arbitraries with ScalaCheck {
  ...
  "CatalogSearchCriterion" should {
    ...
    "support simple searches" in {
      ...
      matcher.matches(SiderealTarget("testObj", base, List(new Magnitude(3.0, MagnitudeBand.J)))) should beFalse
      ...
      matcher.matches(SiderealTarget("testObj", pos, List(new Magnitude(3.0, MagnitudeBand.J)))) should beTrue
      // mag out of range
      matcher.matches(SiderealTarget("testObj", pos, List(new Magnitude(11.0, MagnitudeBand.J)))) should beFalse
    }
    "support search with offset" in {
      ...
      matcher.matches(SiderealTarget("testObj", base, List(new Magnitude(3.0, MagnitudeBand.J)))) should beTrue
      ...
      matcher.matches(SiderealTarget("testObj", pos2, List(new Magnitude(3.0, MagnitudeBand.J)))) should beTrue
    }
    "support search with offset and pos angle" in {
      ...
      val pos = Coordinates(RightAscension.fromAngle(baseRA), Declination.fromAngle(baseDec + Angle.fromArcmin(11.0)).getOrElse(Declination.zero))
      matcher.matches(SiderealTarget("testObj", pos, List(new Magnitude(3.0, MagnitudeBand.J)))) should beFalse
