...
object GemsMagnitudeTable extends MagnitudeTable {
    ...
    def lookup(site: Site): Option[MagnitudeCalc] =
      ((site, probe) match {
        case (Site.GS, odgw) if odgw.isInstanceOf[GsaoiOdgw] =>
          Some(GsaoiOdgwMagnitudeLimitsCalculator.getGemsMagnitudeConstraints(GemsGuideStarType.flexure, MagnitudeBand.H.some))

        case (Site.GS, can) if can.isInstanceOf[Canopus.Wfs] =>
          Some(CanopusWfsMagnitudeLimitsCalculator.getGemsMagnitudeConstraints(GemsGuideStarType.tiptilt, MagnitudeBand.R.some))
  ...
  trait LimitsCalculator {
    ...
    def adjustGemsMagnitudeLimitsForJava(starType: GemsGuideStarType, nirBand: edu.gemini.shared.util.immutable.Option[Band], conditions: Conditions): MagnitudeConstraints =
      getGemsMagnitudeConstraints(starType, nirBand.asScalaOpt.map(_.toNewModel)).map(m => conditions.magAdjustOp.apply(m.toOldModel).toNewModel)

    def searchCriterionBuilder(name: String, radiusLimit: skycalc.Angle, instrument: GemsInstrument, magConstraint: MagnitudeConstraints, posAngles: java.util.Set[skycalc.Angle]): CatalogSearchCriterion = {
      ...
      CatalogSearchCriterion(name, magConstraint, radiusConstraint, searchOffset.asScalaOpt.map(_.toNewModel), searchPA.map(_.toNewModel))
    }
  ...
  private lazy val GsaoiOdgwMagnitudeLimitsCalculator = new LimitsCalculator {
    ...
    override def getGemsMagnitudeConstraints(starType: GemsGuideStarType, nirBand: Option[MagnitudeBand]): MagnitudeConstraints= {
  ...
  lazy val CanopusWfsMagnitudeLimitsCalculator = new CanopusWfsCalculator {
    override def getGemsMagnitudeConstraints(starType: GemsGuideStarType, nirBand: Option[MagnitudeBand]) =
