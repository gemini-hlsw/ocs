...
public class ImprovedSkyCalcMethods {
    private static final Logger LOG = Logger.getLogger(ImprovedSkyCalcMethods.class.getName());
    ...
    /**
     * Returns the altitude in deg for the given airmass
     */
    public static double getAltitude(double airmass) {
        if (airmass == 0.) return 0; // XXX?
        return Math.toDegrees(Math.asin(1./airmass));
    }
     ...
     */
    public static JulianDate jd_sun_alt(double alt, JulianDate jdguess, double lat, double longit) {
        double del = 0.002;

        /* first guess */

        Coordinates sun = lpsun(jdguess);
        double ra  = sun.getRaDeg();
        double dec = sun.getDecDeg();

        double ha = lst(jdguess,longit) - ra;
        double alt2 = altit(dec,ha,lat);
        jdguess = new JulianDate(jdguess.toDouble() + del);

        sun = lpsun(jdguess);
        ra  = sun.getRaDeg();
        dec = sun.getDecDeg();

        double alt3 = altit(dec,(lst(jdguess,longit) - ra),lat);
        double err = alt3 - alt;
        double deriv = (alt3 - alt2) / del;
        short i =0;

        while((Math.abs(err) > 0.1) && (i < 10)) {
            jdguess = new JulianDate(jdguess.toDouble() - err/deriv);
            sun = lpsun(jdguess);
            ra  = sun.getRaDeg();
            dec = sun.getDecDeg();
            ...
            if (i == 9) {
                LOG.log(Level.WARNING, "Sunrise, set, or twilight calculation not converging!");
            }
        }
    ...
    public static double ha_alt(double dec, double lat, double alt) {
        ...
        if (Math.abs(x) <= 1.) {
            ...
            LOG.log(Level.WARNING, "Error in ha_alt ... acos(>1).");
            return 1000.;
        }
    }
    ...
    public static double[] min_max_alt(double lat, double dec) {
        ...
        double x = Math.cos(dec) * Math.cos(lat) + Math.sin(dec) * Math.sin(lat);
        if (Math.abs(x) <= 1.) {
            max = Math.asin(x) * DEG_IN_RADIAN;
        } else {
            LOG.log(Level.WARNING, "Error in min_max_alt -- arcsin(>1)");
     ...
     * Low precision sun.
     */
    public static Coordinates lpsun(JulianDate jd) {
        double n = jd.toDouble() - JulianDate.J2000.toDouble();
        double L = 280.460 + 0.9856474 * n;
        double g = (357.528 + 0.9856003 * n)/DEG_IN_RADIAN;
        double lambda = (L + 1.915 * Math.sin(g) + 0.020 * Math.sin(2. * g))/DEG_IN_RADIAN;
        ...
        double ra = (atan_circ(x,y))*HRS_IN_RADIAN;
        double dec = (Math.asin(z))*DEG_IN_RADIAN;
    ...
    public static double altit(double dec, double ha, double lat) {
        ...
        DoubleRef parang = new DoubleRef();
    ...
    public static double lst(JulianDate jd, double longit) {
        return lst(jd.toDouble(), longit);
    }
	...
	//      istar=         10^ (-0.4d*(3.84d + 0.026d*abs(mpa) + (4.d-9)*          mpa^ 4))
	        double istar=Math.pow(10, (-0.4d*(3.84d + 0.026d*abs(mpa) + (4.e-9)* Math.pow(mpa, 4))));
            double fp;
	        if (mdist >= 10.) {
