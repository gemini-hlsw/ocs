 ...
 */
public class Michelle_SciAreaFeature extends SciAreaFeatureBase {
     ...
     */
    public Michelle_SciAreaFeature() {
    }

    private InstMichelle getMichelle() {
        return getContext().instrument().orNull(InstMichelle.SP_TYPE);
    }
    ...
    protected boolean _calc(TpeImageInfo tii)  {
        ...
        if (SciAreaFeature.getDisplayChopBeams()) {
            ...
            if (inst.getDisperser().getMode() == MichelleParams.DisperserMode.CHOP) {
                ...
                _chopTrans1.setToTranslation(-_nodChopOffset.x, -_nodChopOffset.y);
                _chopTrans2.setToTranslation(0, 0);
            }
        }
