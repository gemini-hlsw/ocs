 ...
 * ESO Archive
 *
 * $Id: ImageProcessor.java 6525 2005-08-03 21:13:39Z brighton $
 *
 * who             when        what
 * --------------  ----------  ----------------------------------------
...
public class ImageProcessor {
    ...
    // Value of the BITPIX property, if defined
    private double _bitpix = 1.;
    ...
    public void setSourceImage(PlanarImage sourceImage, ImageProcessor imageProcessor) {
        ...
        if (_sourceImage == null) {
            _displayImage = _rescaledSourceImage = null;
            return;
        }
...
//                // 95% seems to be a better default than median filter
//                autoSetCutLevels(95., region);
            } else {
    ...
    public void update() {
        ...
        try {
            ...
            if (_numBands == 1) {
                ...
                PlanarImage _byteImage = ImageOps.lookup(_shortImage, _scaleLookupTable);
    ...
    private void calculateImageStatistics(Rectangle2D.Double region) {
        ...
        if (_dataMin != _dataMax) {
            ...
            _minValue = _dataMin;
            _maxValue = _dataMax;
        } else if (Float.isNaN(_blank) && _bitpix != -32) {
