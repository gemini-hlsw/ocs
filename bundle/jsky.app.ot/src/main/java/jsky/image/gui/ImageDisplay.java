...
public class ImageDisplay extends JComponent
    ...
    public synchronized void paintComponent(Graphics g) {
        ...
        Rectangle2D region = _displayImage.getBounds().createIntersection(clipBounds);
        g2D.setClip(region);
        ...
        for (tj = tymin; tj <= tymax; tj++) {
            for (ti = txmin; ti <= txmax; ti++) {
                ...
                WritableRaster wr = Raster.createWritableRaster(_sampleModel, dataBuffer, null);
