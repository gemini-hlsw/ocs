...
public class DivaGraphicsImageDisplay extends JCanvas implements GraphicsImageDisplay {
    ...
    public synchronized void paintLayer(Graphics2D g2D, Rectangle2D region) {
        ...
        Rectangle2D imageBounds = _displayImage.getBounds().createIntersection(clipBounds);
        g2D.setClip(imageBounds);
        if (region == null) {
            region = imageBounds;
        } else {
            region = region.createUnion(imageBounds);
        }
        ...
        for (tj = tymin; tj <= tymax; tj++) {
            for (ti = txmin; ti <= txmax; ti++) {
                ...
                WritableRaster wr = WritableRaster.createWritableRaster(_sampleModel, dataBuffer, null);
        ...
        g2D.setClip(clipBounds); // restore previous clip bounds
     ...
     * @param band the band of the image (0 for FITS files)
     */
    private float _getPixelValue(PlanarImage im, int ix, int iy, int w, int h, int band) {
        ...
        int x = (int) ((double) ix / (_tileWidth * subsample));
        int y = (int) ((double) iy / (_tileHeight * subsample));
        if (x < 0 || y < 0) {
            return 0.0F;
        }
