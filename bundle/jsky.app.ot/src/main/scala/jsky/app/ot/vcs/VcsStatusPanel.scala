...
class VcsStatusPanel(tracker: VcsStateTracker) extends BorderPanel {
  ...
  def handle(evt: VcsStateEvent): Unit = evt match {
    ...
    case VcsStateEvent(_, _, UpToDate, _) => show(BG_GREY, blankIcon, " ")
    case VcsStateEvent(_, _, s, _)        =>
      val (color, icon, text) = status(s)
      show(color, icon, text)
  }

  def status(s: ProgramStatus): (Color, Icon, String) =
    s match {
      case Unknown        => (LIGHT_SALMON, alertIcon, StatusUnknownMessage)
      case PendingSync    => (CANTALOUPE, alertIcon, PendingSyncMessage)
      case PendingUpdate  => (BANANA, alertIcon, PendingFetchMessage)
      case PendingCheckIn => (BANANA, alertIcon, PendingStoreMessage)
