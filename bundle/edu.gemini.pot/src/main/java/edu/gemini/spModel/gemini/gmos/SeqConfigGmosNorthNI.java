...
//
// $Id: SeqConfigGmosNorthNI.java 46768 2012-07-16 18:58:53Z rnorris $
//
 ...
 */
public class SeqConfigGmosNorthNI implements ISPNodeInitializer {
    ...
    public void initNode(ISPFactory factory, ISPNode node)
             {
        //System.out.println("Initializing a GMOS Seq Comp");

        ISPSeqComponent castNode = (ISPSeqComponent) node;
        if (!castNode.getType().equals(SeqConfigGmosNorth.SP_TYPE)) {
            throw new InternalError();
        }

        // data object of this Seq Component.
        try {
            castNode.setDataObject(new SeqConfigGmosNorth());
        } catch (Exception ex) {
            System.out.println("Failed to set data object of SeqConfigGMOS node");
        }
    ...
    public void updateNode(ISPNode node)  {
        ...
        node.putClientData(IConfigBuilder.USER_OBJ_KEY,
                           new SeqConfigGmosNorthCB((ISPSeqComponent) node));
    }
}
