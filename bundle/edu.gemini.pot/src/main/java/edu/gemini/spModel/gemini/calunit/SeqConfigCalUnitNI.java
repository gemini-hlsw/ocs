...
//
// $Id: SeqConfigCalUnitNI.java 46768 2012-07-16 18:58:53Z rnorris $
//
 ...
 */
public class SeqConfigCalUnitNI implements ISPNodeInitializer {
    /**
    ...
    public void initNode(ISPFactory factory, ISPNode node)
             {
        //System.out.println("Initializing a CalUnit Seq Comp");

        ISPSeqComponent castNode = (ISPSeqComponent) node;
        if (!castNode.getType().equals(SeqConfigCalUnit.SP_TYPE)) {
            throw new InternalError();
        }

        // data object of this Seq Component.
        try {
            castNode.setDataObject(new SeqConfigCalUnit());
        } catch (Exception ex) {
            System.out.println("Failed to set data object of SeqConfigCalUnit node");
        }
    ...
    public void updateNode(ISPNode node)  {
        ...
        node.putClientData(IConfigBuilder.USER_OBJ_KEY,
                           new SeqConfigCalUnitCB((ISPSeqComponent) node));
    }
}
