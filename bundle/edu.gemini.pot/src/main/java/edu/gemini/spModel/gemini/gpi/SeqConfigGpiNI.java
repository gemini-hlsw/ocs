 ...
 */
public class SeqConfigGpiNI implements ISPNodeInitializer {
    ...
    public void initNode(ISPFactory factory, ISPNode node)
        ...
        if (!castNode.getType().equals(SeqConfigGpi.SP_TYPE)) {
            throw new InternalError();
        }

        // data object of this Seq Component.
        try {
            castNode.setDataObject(new SeqConfigGpi());
        } catch (Exception ex) {
            System.out.println("Failed to set data object of SeqConfigGpi node");
        }
    ...
    public void updateNode(ISPNode node)  {
        ...
        node.putClientData(IConfigBuilder.USER_OBJ_KEY,
                           new SeqConfigGpiCB((ISPSeqComponent) node));
    }
}
