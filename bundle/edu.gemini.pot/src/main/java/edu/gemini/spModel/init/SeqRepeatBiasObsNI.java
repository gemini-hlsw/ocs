...
//
// $Id: SeqRepeatBiasObsNI.java 46768 2012-07-16 18:58:53Z rnorris $
//
...
import edu.gemini.spModel.seqcomp.SeqRepeatBiasObsCB;
 ...
 */
public class SeqRepeatBiasObsNI implements ISPNodeInitializer {
    private static final Logger LOG = Logger.getLogger(SeqRepeatBiasObsNI.class.getName());
    ...
    public void initNode(ISPFactory factory, ISPNode node)
             {
        //LOG.log(Level.INFO, "Initing Seq Bias Node");

        ISPSeqComponent castNode = (ISPSeqComponent) node;
        if (!castNode.getType().equals(SeqRepeatBiasObs.SP_TYPE)) {
            throw new InternalError();
        }

        // data object of this Seq Component.
        try {
            castNode.setDataObject(new SeqRepeatBiasObs());
        } catch (Exception ex) {
            LOG.log(Level.WARNING, "Failed to set data object of SpRepeatBiasObs node", ex);
        }
    ...
    public void updateNode(ISPNode node)  {
        node.putClientData(IConfigBuilder.USER_OBJ_KEY,
                           new SeqRepeatBiasObsCB((ISPSeqComponent) node));
    }
}
