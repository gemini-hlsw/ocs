...
//
// $Id: SeqConfigNIFSNI.java 46768 2012-07-16 18:58:53Z rnorris $
//
 ...
 */
public class SeqConfigNIFSNI implements ISPNodeInitializer {
    ...
    public void initNode(ISPFactory factory, ISPNode node)
             {
        //System.out.println("Initializing a NIFS Seq Comp");

        ISPSeqComponent castNode = (ISPSeqComponent) node;
        if (!castNode.getType().equals(SeqConfigNIFS.SP_TYPE)) {
            throw new InternalError();
        }

        // data object of this Seq Component.
        try {
            castNode.setDataObject(new SeqConfigNIFS());
        } catch (Exception ex) {
            System.out.println("Failed to set data object of SeqConfigNIFS node");
        }
    ...
    public void updateNode(ISPNode node)  {
        ...
        node.putClientData(IConfigBuilder.USER_OBJ_KEY,
                           new SeqConfigNIFSCB((ISPSeqComponent) node));
    }
}
