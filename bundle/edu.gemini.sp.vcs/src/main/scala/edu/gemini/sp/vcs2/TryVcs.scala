...
object VcsFailure {
  ...
  def explain(f: VcsFailure, id: SPProgramID, opName: String, peer: Option[Peer]): String = {
    ...
    val (msg, ex) = f match {
      ...
      case VcsException(ex0)              =>
        ("Something went wrong in the database server: %s".format(Option(ex0.getMessage).getOrElse("unknown internal server failure")), Some(ex0))
      case NeedsUpdate                   =>
      ...
      case Unexpected(msg0)               =>
        ("The changes in the database could not be merged with your version of the program: " + msg0, None)
    }
