...
public final class TRecsRecipe implements ImagingRecipe, SpectroscopyRecipe {
    ...
    private ObservationDetails correctedObsDetails(final TRecsParameters tp, final ObservationDetails odp) {
        ...
        if (odp.calculationMethod() instanceof ImagingExposureCount) {
            return new ObservationDetails(
                    new ImagingExposureCount(((ImagingExposureCount) odp.calculationMethod()).sigma(), correctedExposureTime, odp.coadds(), odp.sourceFraction(), odp.offset()),
                    odp.analysisMethod()
            );
