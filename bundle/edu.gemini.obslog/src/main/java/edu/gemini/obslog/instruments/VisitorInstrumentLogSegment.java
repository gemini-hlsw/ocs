 ...
 */
public class VisitorInstrumentLogSegment extends InstrumentLogSegment {
    public static final Logger LOG = Logger.getLogger(VisitorInstrumentLogSegment.class.getName());

    private static final String NARROW_TYPE = "Visitor";
    ...
    public VisitorInstrumentLogSegment(List<OlLogItem> logItems, OlLogOptions obsLogOptions) {
    ...
    private void _decorateExposureTime(ConfigMap map) {
        ...
        if (exposureTimeValue == null) {
            LOG.severe("No exposureTime property in Visitor Instrument items");
        } else {
    ...
    private static class NameLoggableSpType implements LoggableSpType {
        ...
        @Override
        public String name() {
            return VisitorInstrument.NAME_PROP.getDisplayName();
        }
