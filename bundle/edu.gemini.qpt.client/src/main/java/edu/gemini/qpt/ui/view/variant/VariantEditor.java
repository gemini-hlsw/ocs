...
public class VariantEditor extends JDialog {
	...
	public VariantEditor(Frame owner) {
		...
		add(new JPanel(new BorderLayout(8, 8)) {{
			...
			class CondComboBox extends JComboBox {
...
//					NewDialog.this.getRootPane().set;
	...
	public int showEditor(String name, byte cc, byte wv, byte iq, ApproximateAngle wind, Boolean lgs) {
		text.setText(name);
		text.selectAll();
	...
	public int showEditor(Variant v) {
	...
	public int showNew(String name, byte cc, byte wv, byte iq) {
		setTitle("Add Variant");
		return showEditor(name, cc, wv, iq, null, false);
	}

	public Conds getVariantConditions() {
	...
	public ApproximateAngle getVariantWindConstraint() {
		return variantWindConstraint;
	}

    public Boolean getVariantLgsConstraint() {
