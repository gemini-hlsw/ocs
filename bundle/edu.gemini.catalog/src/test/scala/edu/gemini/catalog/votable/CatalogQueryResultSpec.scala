...
class CatalogQueryResultSpec extends SpecificationWithJUnit {
  ...
  "CatalogQueryResultSpec" should {
    "be able to filter targets inside the requested range limit" in {
      val qc = CatalogQuery(0, c, RadiusConstraint.between(Angle.zero, coneSearch), Some(noMagnitudeConstraint))
    ...
    "be able to filter targets to a specific ring" in {
      val qc = CatalogQuery(0, c, RadiusConstraint.between(Angle.fromDegrees(0.05), coneSearch), Some(noMagnitudeConstraint))
    ...
    "be able to filter by faintness on band j" in {
      ...
      val qc = CatalogQuery(0, c, RadiusConstraint.between(Angle.zero, coneSearch), Some(m))
      val filtered = unfiltered.filter(qc)
