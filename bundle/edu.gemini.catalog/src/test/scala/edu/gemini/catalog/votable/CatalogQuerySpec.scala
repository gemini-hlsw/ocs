...
class CatalogQuerySpec extends SpecificationWithJUnit {
  ...
  val par10_10 = CatalogQuery.catalogQueryOnBand(base, rad10, mag10)
  val par5_10 = CatalogQuery.catalogQueryOnBand(base, rad5, mag10)
  ...
  "CatalogQuerySpec" should {
    ...
    "be a superset within the range limits" in {
      ...
      coordinates.map(c =>
        par10_10 should beSuperSetOf(CatalogQuery.catalogQueryOnBand(c, rad5, mag10))
      )
    }
    "be a superset at the pole" in {
      ...
      val poleP = CatalogQuery.catalogQueryOnBand(pole, rad10, mag10)
      // 5 arcsec radius close to the pole
      val closeP = CatalogQuery.catalogQueryOnBand(close, rad5, mag10)
    ...
    "not be a superset far of the pole" in {
      ...
      val farP = CatalogQuery.catalogQueryOnBand(far, rad5, mag10)
    ...
    "not be a superset out of the range limits" in {
      ...
      coordinates.map(c =>
        par10_10 should beNoSuperSetOf(CatalogQuery.catalogQueryOnBand(c, rad5, mag10))
      )
    }
    "not be a supersef far out of range" in {
      val far = CatalogQuery.catalogQueryOnBand(Coordinates(RightAscension.fromDegrees(180), Declination.zero), rad5, mag10)
