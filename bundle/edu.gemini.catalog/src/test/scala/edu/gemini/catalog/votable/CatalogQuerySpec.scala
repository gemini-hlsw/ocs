...
class CatalogQuerySpec extends SpecificationWithJUnit {
  ...
  "CatalogQuerySpec" should {
    ...
    "be a superset within the range limits" in {
      ...
      coordinates.map(c =>
        par10_10 should beSuperSetOf(CatalogQuery.catalogQuery(c, rad5, mag10, ucac4))
      )
    }
    "be a superset at the pole" in {
      ...
      val poleP = CatalogQuery.catalogQuery(pole, rad10, mag10, ucac4)
      // 5 arcsec radius close to the pole
      val closeP = CatalogQuery.catalogQuery(close, rad5, mag10, ucac4)
    ...
    "not be a superset far of the pole" in {
      ...
      val farP = CatalogQuery.catalogQuery(far, rad5, mag10, ucac4)
    ...
    "not be a superset out of the range limits" in {
      ...
      coordinates.map(c =>
        par10_10 should beNoSuperSetOf(CatalogQuery.catalogQuery(c, rad5, mag10, ucac4))
      )
    }
    "not be a supersef far out of range" in {
      val far = CatalogQuery.catalogQuery(Coordinates(RightAscension.fromDegrees(180), Declination.zero), rad5, mag10, ucac4)
