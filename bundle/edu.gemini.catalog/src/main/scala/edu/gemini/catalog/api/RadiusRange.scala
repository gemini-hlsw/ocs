 ...
 */
object RadiusRange {
  val empty = RadiusRange(Angle.zero, Angle.zero)

  /**
   * Constructs a range between 2 angles. It will produce the correct ordering to create a valid range
   */
  def between(minLimit: Angle, maxLimit: Angle): RadiusRange = {
    if (minLimit < maxLimit) {
      RadiusRange(minLimit, maxLimit)
    } else {
      RadiusRange(maxLimit, minLimit)
    }
  }
}

case class RadiusRange private (minLimit: Angle, maxLimit: Angle) {
   ...
   */
  def adjust(offset: Offset): RadiusRange = {
