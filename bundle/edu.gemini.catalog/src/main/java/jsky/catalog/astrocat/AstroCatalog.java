...
public class AstroCatalog implements PlotableCatalog {
    ...
    // If _path is overwritten by a detailed description from another xml file, this contains the original value of _path
    ...
    /**
     *  Returns the path that was originally in the AstroCat.xml file.
     *  If this is an XML file, the path is overwritten with the value
     *  defined there. This value is needed when saving a copy of
     *  Astrocat.xml in ~/.jsky/ with the user's changes.
     */
    ...
    public FieldDesc getParamDesc(String name) {
        if (_dummyParamDesc == null)
            _initSearchParameters();
        for (FieldDesc desc : _dummyParamDesc)
    ...
    private void _setQueryRegion(QueryArgs queryArgs, SearchCondition[] sc) throws CatalogException, IOException {
    ...
    private QueryResult _httpQuery(QueryArgs queryArgs) throws CatalogException, IOException {
        URL queryUrl = _getQueryUrl(queryArgs);

        LOG.log(Level.FINE, "URL = " + queryUrl);
    ...
    // parameters sent to the server. For catalogs that can be searched by center
    ...
    private void _checkForDetailedCatalogDesc() {
        ...
        // XXX REL-236 bugfix for user's existing ~/.jsky/AstroCat.xml file contents,
        // which may contain wrong path for 2MASS image server
        if ("/cgi-bin/Oasis/2MASSImg/nph-2massimg".equals(_path)) {
            _path = "2MASSImg.xml";
        }
            ...
            try {
                ...
                if (filename != null) {
                    ...
                    if (!file.exists()) {
                        ...
                        URL defaultURL = AstroCatalog.class.getResource("/resources/conf/" + _path);
                        if (defaultURL != null)
                            url = defaultURL;
                    }
                }
