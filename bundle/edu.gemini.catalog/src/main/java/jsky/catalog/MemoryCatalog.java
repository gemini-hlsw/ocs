...
public class MemoryCatalog extends DefaultTableModel
    ...
    private final Option<SkycatOutputAdapter.Factory> adapterFactoryOpt;
     ...
     */
    public MemoryCatalog(Catalog catalog, InputStream in, Option<SkycatOutputAdapter.Factory> factoryOpt, int maxRows) throws IOException {
        _catalog = catalog;
        adapterFactoryOpt = factoryOpt;
        _init(in, maxRows);
    ...
    protected void _init(InputStream in, int maxRows) throws IOException {
        ...
        if (adapterFactoryOpt.isEmpty()) {
            adapt = new DefaultOutputAdapter(reader);
        } else {
            adapt = adapterFactoryOpt.getValue().create(reader);
        }
    ...
    protected Vector<Object> _parseRow(String lineStr) {
        ...
        int i = 0;
        Vector<Object> row = new Vector<>(numCols, 1);
     ...
     */
    protected Vector<String> makeColumnIdentifiers(FieldDesc[] fields) {
        Vector<String> v = new Vector<>(fields.length, 1);
        for (FieldDesc field : fields) {
            String label = field.getName();
            v.add(label != null ? label : "");
        }
        return v;
    }
    ...
    public FieldDesc getParamDesc(String name) {
        for (FieldDesc _field : _fields)
            if (_field != null && _field.getName().equals(name))
                return _field;
        return null;
    }
    ...
    public RowCoordinates getRowCoordinates() {
        ...
        if (_catalog instanceof SkycatCatalog) {
            SkycatConfigEntry e = ((SkycatCatalog) _catalog).getConfigEntry();
            _rowCoordinates = e.getRowCoordinates();
            if (_rowCoordinates != null) return _rowCoordinates;
        }
