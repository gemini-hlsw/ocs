 ...
 *
 * @version $Revision: 47126 $ $Date: 2012-08-01 08:40:43 -0700 (Wed, 01 Aug 2012) $
...
public class SkycatConfigEntry {
    ...
    /** parameter: faintest star to return from a query. */
    public static final String FAINTEST = "Faintest";

    /** parameter: brightest star to return from a query. */
    public static final String BRIGHTEST = "Brightest";

    /** parameter: the band used to check magnitude ranges. */
    ...
    protected void determineSearchParameters() {
        ...
        if (_servType.equals("namesvr")) {
            ...
            if (isCatalog) {
                ...
                p.setDescription("The faintest star to return");
                p.setFieldClass(Float.class);
                p.setDefaultValue(new Float(20.0));
                params.add(p);

                p = new FieldDescAdapter(BRIGHTEST);
                p.setDescription("The brightest star to return");
                p.setFieldClass(Float.class);
                p.setDefaultValue(new Float(-2.0));
                params.add(p);
