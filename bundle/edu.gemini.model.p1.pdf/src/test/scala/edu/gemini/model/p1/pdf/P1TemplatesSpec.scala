...
class P1TemplatesSpec extends Specification with XmlMatchers {
  ...
  "The P1 DEFAULT Template" should {
    ...
    "present the correct name when using IGRINS, REL-3714" in {
    ...
    "display reviewer for FT, REL-1894" in {
      val result = transformProposal("proposal_fast_turnaround.xml", P1PDF.NOIRLabDARP.copy(investigatorsList = InvestigatorsListOption.DefaultList))
      val proposalXml = XML.loadString(result)
