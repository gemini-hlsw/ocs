...
class ShellAdvisor(
  ...
  def open(_context: RichShellContext[Model]) {
    ...
    context.context.addView(new BoundView.Advisor("Overview", proposalView), "proposal", NorthOf, "problems")
    context.context.addView(new BoundView.Advisor("Band 3", obsListViewB3), "band3", EastOf, "proposal")
    context.context.addView(new BoundView.Advisor("Time Requests", partnerView), "partners", Beneath, "proposal")
    context.context.addView(new BoundView.Advisor("Scheduling", schedulingView), "scheduling", Beneath, "proposal")
    context.context.addView(new BoundView.Advisor("Submit", submitView), "submit", Beneath, "partners")
    context.context.addView(new BoundView.Advisor("Observations", obsListView), "obs", Above, "band3")
    context.context.addView(new BoundView.Advisor("Targets", targetView), "targets", Above, "obs")
